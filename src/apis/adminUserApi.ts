...
export const getUserList = async (searchString: string, filters: Record<string, unknown>): Promise<[]> => {
    ...
    return axios.get(`${baseUrl}/list?${params}`).then((res) => res.data).catch(() => []);
};
...
export const getUserReservations = async (id: string): Promise<[]> => {
    return axios.get(`${baseUrl}/${id}/reservation/list`).then((res) => res.data);
};

export const getAllReservations = async (text: string, filters: Record<string, unknown>): Promise<[]> => {
    filters = filterPropertiesNotNull(filters);
    const params = new URLSearchParams({ text, ...filters }).toString();
    return axios.get(`${baseUrl}/reservation/list?${params}`).then((res) => res.data);
};

export const cancelReservation = async (id: string): Promise<[]> => {
    return axios.delete(`${baseUrl}/reservation/${id}/cancel/`).then((res) => res.data);
};
